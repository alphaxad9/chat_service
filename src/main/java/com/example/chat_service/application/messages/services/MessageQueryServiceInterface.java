// chat_service/src/main/java/com/example/chat_service/application/messages/services/MessageQueryServiceInterface.java
...
/**
 * Application-layer interface for message query (read) operations.
 *
 * <p>Orchestrates read-side business logic and coordinates domain entities with infrastructure
 * query repositories. All methods operate on {@link Message} entities (not aggregates) optimized
 ...
 *   <li>All queries automatically exclude messages where {@code isDeleted = true} unless explicitly documented</li>
 ...
 * </ul></p>
 */
public interface MessageQueryServiceInterface {

    // ── Single Entity Queries (Active Messages Only) ──────────────────

    /**
     * Retrieve an active message by its unique ID.
     *
     ...
     * Suitable for: loading message details, permission checks, UI message displays.</p>
     */
    Optional<Message> getMessageById(UUID messageId);

    /**
     * Retrieve a message by its unique ID, including deleted messages.
     *
     * @param messageId the unique identifier of the message
     ...
     * Most application logic should prefer {@link #getMessageById(UUID)}.</p>
     */
    Optional<Message> getMessageByIdIncludingDeleted(UUID messageId);
     ...
     * Returns {@code false} if message doesn't exist OR is deleted.
    ...
    /**
     * Retrieve the most recent active message in a specific room.
     *
     ...
     */
    Optional<Message> getLatestActiveMessageByRoomId(UUID roomId);

    /**
     * Retrieve the most recent active message for multiple rooms in a single query.
     *
     ...
     *
     * <p>More efficient than calling {@link #getLatestActiveMessageByRoomId(UUID)} in a loop —
     * performs a single batched query instead of N+1 queries.</p>
     */
    Map<UUID, Message> getLatestActiveMessagesByRoomIds(Collection<UUID> roomIds);

    /**
     * Retrieve the most recent active message summary for multiple rooms.
     *
     * @param roomIds collection of room identifiers to check
     * @return map of roomId → latest active {@link MessageSummary} (only includes rooms with messages)
     *
     * <p>Optimized projection version of {@link #getLatestActiveMessagesByRoomIds(Collection)}.
     ...
     */
    Map<UUID, MessageSummary> getLatestActiveMessageSummariesByRoomIds(Collection<UUID> roomIds);

    // ── Bulk Queries by Room (Active Messages Only) ───────────────────

    /**
     * Retrieve all active messages in a specific room.
     *
     * @param roomId the unique identifier of the room
     ...
     */
    List<Message> getAllActiveMessagesByRoomId(UUID roomId);

    /**
     * Retrieve active messages in a room, limited by count (most recent first).
     *
     * @param roomId the unique identifier of the room
     ...
     */
    List<Message> getActiveMessagesByRoomIdLimited(UUID roomId, int limit);

    /**
     * Retrieve active messages in a room with cursor-based pagination (newer messages).
     *
     * @param roomId the unique identifier of the room
     ...
     */
    List<Message> getActiveMessagesByRoomIdAfter(UUID roomId, UUID afterId, int limit);

    /**
     * Retrieve active messages in a room before a given cursor (for loading older history).
     *
     * @param roomId the unique identifier of the room
     ...
     */
    List<Message> getActiveMessagesByRoomIdBefore(UUID roomId, UUID beforeId, int limit);
     ...
     */
    long countActiveMessagesByRoomId(UUID roomId);

    // ── Bulk Queries by Sender (Active Messages Only) ─────────────────

    /**
     * Retrieve all active messages sent by a specific user.
     *
     ...
     */
    List<Message> getAllActiveMessagesBySenderId(UUID senderId);

    /**
     * Retrieve active messages sent by a user in specific rooms.
     *
     * @param senderId the unique identifier of the sender
     ...
     */
    List<Message> getActiveMessagesBySenderIdAndRooms(UUID senderId, Collection<UUID> roomIds);
     ...
     */
    long countActiveMessagesBySenderId(UUID senderId);

    // ── Bulk Queries by Status (Active Messages Only) ─────────────────

    /**
     * Retrieve active messages by delivery status.
     *
     ...
     */
    List<Message> getActiveMessagesByStatus(Message.Status status);

    /**
     * Retrieve active messages by status in a specific room.
     *
     ...
     */
    List<Message> getActiveMessagesByRoomAndStatus(UUID roomId, Message.Status status);

    /**
     * Retrieve active SENT messages older than a threshold (potential delivery issues).
     *
     ...
     */
    List<Message> getActiveSentMessagesOlderThan(LocalDateTime olderThan);

    // ── Reply Chain Queries (Active Messages Only) ────────────────────

    /**
     * Retrieve all active reply messages for a given parent message.
     *
     ...
     */
    List<Message> getActiveRepliesTo(UUID parentId);

    /**
     * Retrieve active reply messages for a parent, limited by count.
     *
     * @param parentId the ID of the parent message being replied to
     ...
     */
    List<Message> getActiveRepliesToLimited(UUID parentId, int limit);
     ...
     */
    List<Message> getActiveMessagesByIds(Collection<UUID> messageIds);
     ...
     */
    List<Message> getActiveMessagesByRoomIds(Collection<UUID> roomIds);

    // ── Activity/Time-Based Queries (Active Messages Only) ────────────

    /**
     * Retrieve active messages with recent creation time.
     *
     ...
     */
    List<Message> getActiveMessagesCreatedSince(LocalDateTime sinceTimestamp, int limit);

    /**
     * Retrieve active messages with recent activity (created or updated).
     *
     ...
     */
    List<Message> getActiveMessagesUpdatedSince(LocalDateTime sinceTimestamp, int limit);

    // ── Content/Image Queries (Active Messages Only) ──────────────────

    /**
     * Retrieve active messages that have image attachments in a room.
     *
     * @param roomId the room identifier
     ...
     */
    List<Message> getActiveMessagesWithImagesByRoomId(UUID roomId);

    /**
     * Retrieve active messages that have image attachments by sender.
     *
     ...
     */
    List<Message> getActiveMessagesWithImagesBySenderId(UUID senderId);
     ...
     */
    List<MessageSummary> getActiveMessageSummariesByRoomId(UUID roomId);
     ...
     *
     * <p>Same optimization as {@link #getActiveMessageSummariesByRoomId(UUID)}
     * but for arbitrary message ID lookups. Useful for batch UI rendering.</p>
     */
    List<MessageSummary> getActiveMessageSummariesByIds(Collection<UUID> messageIds);
     ...
     */
    List<MessageSummary> getActiveMessageSummariesByRoomIdAfter(UUID roomId, UUID afterId, int limit);
     ...
     */
    List<MessageSummary> getActiveReplySummariesTo(UUID parentId);

    // ── Read-Side Utility Queries ────────────────────────────────────

    /**
     * Check if a message with the given ID exists and is active.
     *
     * @param messageId the UUID of the message
     * @return {@code true} if message exists and {@code isDeleted = false}
     *
     * <p>Fast existence check without loading full entity.
     * Use for: precondition checks, cache key validation.</p>
     */
    boolean activeMessageExists(UUID messageId);

    /**
     * Get the sender ID of a message by its ID.
     *
     * @param messageId the UUID of the message
     * @return the sender's UUID if message is active, otherwise {@code null}
     *
     * <p>Convenience method for ownership checks without loading full entity.
     * Returns {@code null} if message doesn't exist or is deleted.
     * Suitable for: permission validation, message routing logic.</p>
     */
    UUID getMessageSenderId(UUID messageId);

    /**
     * Get the room ID of a message by its ID.
     *
     * @param messageId the UUID of the message
     * @return the room's UUID if message is active, otherwise {@code null}
     *
     * <p>Convenience method for room context checks without loading full entity.
     * Returns {@code null} if message doesn't exist or is deleted.
     * Suitable for: permission gates, UI conditional rendering.</p>
     */
    UUID getMessageRoomId(UUID messageId);

    /**
     * Get the delivery status of a message by its ID.
     *
     * @param messageId the UUID of the message
     * @return the message's {@link Message.Status} if active, otherwise {@code null}
     *
     * <p>Convenience method for quick status checks without loading full entity.
     * Returns {@code null} if message doesn't exist or is deleted.
     * Suitable for: read receipt UI, delivery monitoring.</p>
     */
    Message.Status getMessageStatus(UUID messageId);

    /**
     * Get the creation timestamp of a message.
     *
     * @param messageId the UUID of the message
     * @return the {@link LocalDateTime} of creation if message is active, otherwise {@code null}
     *
     * <p>Convenience method for sorting/ordering without loading full entity.
     * Returns {@code null} if message doesn't exist or is deleted.
     * Suitable for: message ordering, "recently sent" indicators.</p>
     */
    LocalDateTime getMessageCreatedAt(UUID messageId);
     ...
     *
     * <p>This record mirrors {@link com.example.chat_service.domain.messages.repositories.MessageQueryRepository.MessageSummary}
